...
ncRNA_TSS_df$end[which(ncRNA_TSS_df$sRNA_ID=="ncRv13661")]<-4100987
...
## can i add arnvig citations in df?
...
# add Dinan annotations (these don't use new naming protocol, TSS sites only for bovis and marinum)
# only some are from mtb, no TSS and strand info from mtb
dinan_table <-
  read.delim(
    "Dinan_2014_srnas.csv",
    header = F,
    sep = ",",
    skip = 2,
    stringsAsFactors = F
  )
dinan_df <- select(dinan_table, 1, 2, 3, 4, 5)
head(dinan_df)

for (i in 1:nrow(dinan_df)) {
  if (dinan_df$V2[i] == "M. tuberculosis") {
    ins_line <-
      c("",
        dinan_df$V3[i],
        dinan_df$V4[i],
        ...
        dinan_df$V1[i],
        dinan_df$V5[i])
    ncRNA_df <- rbind(ncRNA_df, ins_line)
  }
}
View(ncRNA_df)

new_df<-ncRNA_df[order(ncRNA_df$annotation),] 
head(new_df)
View(new_df)

# make list of unique ncRNA names from all tables

#### copied from https://github.com/irilenia/baerhunter
...
bam_location = "~/bam_files/PRJNA327080_15"
target_strand = "+"
peak_width = 40
min_UTR_length=50
paired_end_data = TRUE
strandedness = "reversely_stranded"
# # peak_test<-peak_union_calc("PRJNA327080_15", "+", 40, TRUE, "reversely_stranded")
# peak_test_neg<-peak_union_calc("PRJNA327080_15", "-", 40, TRUE, "reversely_stranded")
...
peak_union_calc <- function(bam_location = ".", target_strand, peak_width, paired_end_data = FALSE, strandedness  = "unstranded") {
  ...
  f<-bam_files
  f
  #f<-"~/Data/PRJNA327080_15/SRR3725585_sorted.bam"
...
annotation_file<-"Mtb_h37rv.ASM19595v2_AL123456.3.gff3"
original_sRNA_annotation<-"ncRNA"
annot_file_directory = "ref_seqs/"
...
head(major_f)
m_s_f_pos<-major_features("Mtb_h37rv.ASM19595v2_AL123456.3.gff3", ".", "+", "unknown")
...
target_strand="+"
major_strand_features<-m_strand_features
union_peak_ranges<-peak_union
...
sRNA_calc <- function(major_strand_features, target_strand, union_peak_ranges) {
  ...
  subs_over_df<-as.data.frame(subsetByOverlaps(union_peak_ranges, strand_IRange, maxgap = 1L))
  ...
  head(non_overlap)
  IGR_sRNAs2<- union_peak_ranges[! u_p_IRange %in% as.data.frame(subs_over),]
  IGR_sRNAs <- union_peak_ranges[! u_p_IRange %in% subs_over_df,]
  IGR_sRNAsO<- union_peak_ranges[! union_peak_ranges %in% subsetByOverlaps(union_peak_ranges, strand_IRange, maxgap = 1L),]
  
  head(IGR_sRNAs2)
  head(IGR_sRNAs)
  
  ## only the final line gives correct ranges.
  head(IGR_sRNAsO)
...
head(IGR_sRNAs)
wsrna<-width(IGR_sRNAs)
wsrna
min(wsrna)   #67
max(wsrna)   #3762 (weird outlier--most under 500)
median(wsrna)  #324.5
mean(wsrna)    #380.6

hist(wsrna)

#sRNAs_test<-sRNA_calc(m_s_f_pos, "+", peak_test)
#sRNAs_neg<-sRNA_calc(m_s_f_neg, "-", peak_test_neg)
## changed one line in function: UTRs <- split_features[! (split_features %in% subsetByOverlaps(split_features, strand_IRange))]
#utrs_test<-UTR_calc(m_s_f_pos, "+", peak_test, 50)
#utrs_neg<-UTR_calc(m_s_f_neg, "-", peak_test_neg, 50)
...
w<-width(UTRs)
w
min(w)  #50 (which is setting for min utr length)
max(w)   #2087
mean(w)
median(w)
# [1] 168.5
# still returning quite long utrs
...
# add a line to indicate parameters used to originate the file--won't be necessary
# if calculate parameters itself, but maybe even more useful? peak union function 
# needs to pass on a parameters string? But this would be different for each string
# in each sample of the dataset, so not feasible.
#p_info<- paste("# low_cutoff=", low_coverage_cutoff, " high_cutoff=",high_coverage_cutoff,
#               " min sRNA len=",peak_width, " min UTR_len=",min_UTR_length )
#p_info
...
#' @export
#' 
#' ## removed input parameters of low coverage and high coverage, this is calculated
#' in the peak union calc function
#' 
feature_file_editor <- function(bam_directory = ".", 
  ...
  # why does this function use min_sRNA_length vs min_UTR_length? why different for each strand?
  ...
  # add a line to indicate parameters used to originate the file?
  #p_info<- c("# ", low_coverage_cutoff, " ",high_coverage_cutoff," ",peak_width)
  #p_info
...
test_features<-(feature_file_editor(bam_directory="PRJNA327080_15",
                                    ...
                                    paired_end_data=TRUE, 
                                    strandedness ="reversely_stranded"))
## successfully made new annotation file
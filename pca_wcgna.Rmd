...
![clustering tree of MEs](~/git/mtb_modules/Images/clust_eigengenes.png)
...
![heatmaps of 5 largest modules](~/git/mtb_modules/Images/mod_mem_heatmaps.png)
...
```
![heatmap trait/module correlations and p-values](~/git/mtb_modules/Images/labeled_heatmap.png)


![heatmap trait/module correlations/p-values net2](~/git/mtb_modules/Images/labeled_heatmap_net2.png)
...
length(genes_blue)
#578
genes_darkgrey<-colnames(analysis[,moduleColors2=="darkgrey"])
...
Rv3249c_mods <-datKME2["gene:Rv3249c", abs(datKME2["gene:Rv3249c", ]) > 0.5]
Rv3249c_mods
#             MM.purple MM.darkorange2  MM.black MM.darkgrey
#gene:Rv3249c -0.5007611      0.7673765 0.5202443   0.5930313

# get module correlation and pvalue for specific gene in module
kMEdarkgrey_Rv3249c = bicorAndPvalue(analysis[,"gene:Rv3249c"], MEs2$MEdarkgrey)
kMEdarkgrey_Rv3249c$bicor 
kMEdarkgrey_Rv3249c$p

#list genes in darkgrey module with membership > 0.8?
x <- (abs(datKME2$MM.darkgrey)> 0.8)
y<-datKME2$MM.darkgrey[x]
length(y)



# see how many genes related to low iron in cyan
darkgrey_filtered<- abs(GS_iron)> .2 &  abs(datKME2$MM.darkgrey) > 0.5 
table(darkgrey_filtered)
# get list of gene names
KME_filtered_darkgrey<-dimnames(data.frame(analysis))[[2]][darkgrey_filtered]
KME_filtered_darkgrey
# export gene list
...
Write script to assign utrs to genes. See 'utr_gene_assignment.Rmd' for utr assignment. (function==utr_assign.R)
...
change to 3UTR_RvXXXXx or 5UTR_RvXXXXx
...
```{r}
# look at specific gene and module membership
module = "darkgrey"

column = match(module, modNames2);
moduleGenes = moduleColors2==module;

names(analysis)[moduleColors2=="black"]

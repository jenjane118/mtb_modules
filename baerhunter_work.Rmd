...
see baerhunter_testing.R

```{R baerhunter_testing.R}
...
boxplot(t(cutoff_df), show.names=TRUE,ylab="Number of reads in given percentile (raw)", xlab=c("Percentiles of number of reads in sample"))

```


![boxplot for read distr in one sample](Images/read_distr_sample.png)

2) Calculate quantiles for each .bam as it begins loop in baerhunter.

3) Using data for particular genome coverage (see 1), take number of reads in bottom 5-20% (whatever is appropriate acc to 1 above) as the lower/upper parameters. (how to estimate upper parameter?).  ('peak width' parameter==min sRNA length which is entered by user along with min UTR length).

...
new_df<-reshape(pos_per.df, 
        direction = "long",
        varying = list(names(pos_per.df)[1:12]),
        v.names = "Count",
        idvar = "Percent",
        timevar = "Sample",
        times = 1:12)

new_df$Strand<-rep("Pos", nrow(new_df))
View(new_df)

neg_per.df<-read.delim("percentiles/PRJNA390669_12_neg.tsv", sep = " ",
                       stringsAsFactors = F)

# neg_long<-reshape(neg_per.df,
#                   direction="long",
#                   varying = list(names(neg_per.df)[1:12]),
#                   v.names = "Count",
#                   idvar = "Percent",
#                   timevar = "Sample",
#                   times = 1:12)
# neg_long$Strand<-rep("Neg", nrow(neg_long))
# new_df<-rbind(new_df, neg_long)



png("Images/high_depth_distr.png", width=600, height=600)
...
par(mfrow = c(1,1))
png("Images/med_depth_distr.png", width=600, height = 600)
boxplot(t(pos_per.df), show.names=TRUE,
        main = paste("Percentile distrbution in Dataset",dataset),
        ylab="Number of reads in given percentile (raw)",
        ylim = c(1,1500),
...
Did this--look at presrat_compare.R, no overlap.

22Feb

Look at some of long sRNAs on artemis, decide on filter step. (before merging gffs, or even in BH, before creating gff)

![long RNA bam view](~/git/mtb_modules/Images/longRNA110564.png)

Looking at what datasets have the really long gffs. I suspect they are mostly coming with PRJNA390669_12.

Check numbers of long sRNAs above before merging. (long_srnas.R)
...
1  AL123456.3   50578   51786  1209      + putative_sRNA    NA
2  AL123456.3  630290  631934  1645      - putative_sRNA    NA
3  AL123456.3  987231  988259  1029      - putative_sRNA    NA
4  AL123456.3 1136883 1137892  1010      + putative_sRNA    NA
5  AL123456.3 1271147 1272157  1011      - putative_sRNA    NA
6  AL123456.3 1477072 1481201  4130      + putative_sRNA    NA
7  AL123456.3 1482881 1484082  1202      + putative_sRNA    NA
8  AL123456.3 1960675 1962115  1441      + putative_sRNA    NA
9  AL123456.3 2336631 2337976  1346      - putative_sRNA    NA
10 AL123456.3 2465476 2466983  1508      - putative_sRNA    NA
```

These 10 from PRJNA327080_15 are also in the 45 predicted by PRJNA390669_12, only 2 in the largest dataset PRJNA278760_22. Increase value of high cutoff by *3 instead of 2 and see if that helps get rid of long sRNAs.

Changed script inside baerhunter directory in my local version of BH. Ran using shell script (run_conda_r.sh) to call R script (bh_run_ffe.R):
...
forgot to change script to make new output file, so will be listed as 'output_BH_08_12'. first dataset is PRJNA278760_22_Safa. This didn't work, as already had checked that directory exists. Re-do.

maybe for next ones, just have them iterate through list of datasets?
dataset_list.txt.
...
if (!(dir.exists("./output_BH_28_02/"))) { dir.create("./output_BH_28_02/")
}
...
for (i in 1: length(directory_list)){
  ...
  dataset_name<-paste("output_BH_28_02_", dataset_name, ".gff3", sep="")

# run feature_file_editor to create .gff3 file with new annotations
  feature_file_editor(bam_directory=directory_list[i],
                    original_annotation_file="Mtb_h37rv.ASM19595v2_AL123456.3.gff3",
                    annot_file_dir = "/d/in16/u/sj003/refseqs/Mtb/",
                    output_file= datset_name,
                    original_sRNA_annotation="ncRNA",
                    min_sRNA_length=30,
                    min_UTR_length=50,
                    paired_end_data=TRUE,
...
Ran successfully iterating through all datasets with new high covg parameter of 3* low covg threshold. Now check each gff and see how many srnas predicted.
...
annot <- import.gff3("~/git/mtb_modules/bh_gffs/output_BH_28_02/output_BH_28_02_PRJNA390669_12.gff3")
pred.sRNA <- subset(annot, type == "putative_sRNA")
pred.UTR <- subset(annot, type == "putative_UTR")
w<-width(pred.sRNA)
mean(w)
...
annot <- import.gff3("~/git/mtb_modules/bh_gffs/output_BH_28_02/output_BH_28_02_PRJNA327080_15.gff3")
pred.sRNA <- subset(annot, type == "putative_sRNA")
pred.UTR <- subset(annot, type == "putative_UTR")
w<-width(pred.sRNA)
mean(w)
...
annot <- import.gff3("~/git/mtb_modules/bh_gffs/output_BH_28_02/output_BH_28_02_PRJNA278760_22.gff3")
head(annot)
pred.sRNA <- subset(annot, type == "putative_sRNA")
head(pred.sRNA)
## none present--what went wrong here? (wrong directory structure in shell script)


annot <- import.gff3("~/git/mtb_modules/bh_gffs/output_BH_28_02/output_BH_28_02_PRJEB65014.gff3")
pred.sRNA <- subset(annot, type == "putative_sRNA")
pred.UTR <- subset(annot, type == "putative_UTR")
w<-width(pred.sRNA)
mean(w)
...
# same as before
```
Didn't run on Safa's data because that is slightly different directory structure, not in BWA_mem subdirectory like the others.

Run independently ("output_BH_01_03_PRJNA278760_22.gff3")

Use comb_gff.R but added steps to filter out long srnas before combining. (filtered all >1000)

#how many in combined list are 'confirmed' (i.e. sRNAs predicted by Gerrick and UTRs have TSS)?

```{r}
library(rtracklayer)

res_combined_01_03<-compare_known_with_predicted(annotation_file =                    "~/git/mtb_modules/BH_results/combined_gffs_01_03.gff3", minoverlap = 5L,
                                                 known_RNA_file = "ncRNA_verified.txt"
)
res_combined_01_03
...
Filter for expression? use tpm normalisation in bh

Combine expression data and normalise data (VCF)

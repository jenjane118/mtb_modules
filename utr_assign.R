...
utr_assign <- function(gene_df, refseq_name, annot_file, tss_file) {
  ...
  UTR_df <- data.frame(matrix(0, nrow=length(utrs), ncol=3),stringsAsFactors = F)
  colnames(UTR_df) <- c("strand", "start", "stop")
  ...
  # make id column look like UTR_df format (with ORF id only)
  ref_genes$ID<-sub("gene:", "", ref_genes$ID)

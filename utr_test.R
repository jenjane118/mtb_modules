...
# see how many ties
a<-GenomicRanges::nearest(utr_Granges, ref_granges, select="all", ignore.strand=F)
b<-as.matrix(a)
#show frequency of hits for each utr
c<-as.data.frame(table(b[,1]))
utr_tied<-c$Freq
# see nearest gene (arbitrarily choose in ties)
ng<-GenomicRanges::nearest(utr_Granges, ref_granges, select="arbitrary", ignore.strand=F)

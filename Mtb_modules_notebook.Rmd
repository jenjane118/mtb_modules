...
![paired end reads showing both strands](~/git/mtb_modules/Images/prjna_39066912.27.52.png)

![R1 hidden so you can see whether reads correspond to reverse-strand CDS](~/git/mtb_modules/Images/prjna_390669_12.29.16.png)
...
![from suppl info from baerhunter paper](~/git/mtb_modules/Images/suppl_baerhunter.png)
...
![coverage percentiles](~/git/mtb_modules/Images/PRJNA390669_boxplot.png)
...
![positive strand coverage for PRJNA390669_12](~/git/mtb_modules/Images/PRJNA390669_pos_cov.png)
...
![artemis view of bam read for above sample](~/git/mtb_modules/Images/ARtemis_screenshot_srr5689224.png)
...
![boxplot for coverage percentiles for cds only](~/git/mtb_modules/Images/boxplot_PRJNA390669_cds.png)
...
# rownames = "putative_sRNA:m2031_2292"
...
write.csv(countdata, "count_matrix_complete.csv", quote=F, row.names = T)
...
write.csv(conditions_df, "~/git/mtb_modules/mtb_conditions.txt", header=F, row.names = F, quote=F)
...
control_conds<-c("tyloxapol_pH7.0", "reaeration_day 4", "butyrate_plus_glucose", "dextrose_exponential")
...
dataset<-c(rep("E-MTAB-6011", 3), 
           ...
           rep("GSE67035", 22),
           rep("GSE100097", 12))
# this has 'control' conditions set
control.metadata$study<-dataset
# no control conditions
conditions_df$study<-dataset
...
conditions.coldata<-data.frame(row.names = colnames(countdata), conditions_df$condition)
View(conditions.coldata)



#construct deseq dataset
dds<-DESeqDataSetFromMatrix(countData = countdata,
                              colData = conditions.coldata,
                              #colData = control.coldata,
                              #design = ~control.condition)
                              design = ~conditions_df.condition)

# changed to exclude 'control' from design
...
# use blind=True to ignore 'control' designation
vst_dds<-vst(dds_filtered, blind=TRUE)


# non filtered
#vst_dds<- vst(dds_unfiltered, blind=FALSE)

```

VST (variance stabilising transformation) transformation to normalise data with respect to library size, coverage, etc.
(http://127.0.0.1:27710/library/DESeq2/doc/DESeq2.html#countmat)
...
#boxplot for vst data (filtered)
dds.vst_sum<- assay(vst_dds)

colnames(dds.vst_sum)<-colnames(dds)
par(cex.axis=0.5)
par(mar=c(4,2,1,1))
colors = c(rep("#440154FF",3),rep("#31688EFF",15),rep("#FDE725FF",22),rep("#35B779FF",12))

vst.dds_boxplot <- boxplot(dds.vst_sum,
                           PchCex =0.01,
                           axes=TRUE,
                           las=2,
                           col=colors,
                           outcex=0.35)
legend(2,9, c("E-MTAB-6011", "GEO:GSE83814", "GEO:GSE67035", "GEO:GSE100097"), 
       col=c("#440154FF","#31688EFF","#FDE725FF","#35B779FF"),
       fill=c("#440154FF","#31688EFF","#FDE725FF","#35B779FF"),
       cex=0.75)
```
...
# don't use 'control' conditions, back to sample conditions (conditions_df)

 #PCA plotting with DESeq2 in built function (filtered)
        PCA.prelim <- plotPCA(vst_dds,intgroup="conditions_df.condition")

        #generating PCA table 
        PCA.data <- data.frame(row.names=colnames(countdata),
                               condition=factor(conditions_df$condition),
                               dataset=factor(conditions_df$study))
      
        PCA.data.plot <- prcomp(t(dds.vst_sum))
        ...
        custom <- ggplot(df.vst,aes(x=PC1,y=PC2,color=dataset,shape=group)) + scale_shape_manual(values = 0:21) + geom_point(size=3) + xlab("PC1 (31.8%)") + ylab("PC2 (18.5%)")
...
```{r}
#dendrogram for VST transformed data (filtered)
sizeGrWindow(12,9)
par(cex=0.6)
par(mar=c(5,6,2,0))
...
hc <- hclust(dist(t(dds.vst_sum)),method="average")
...
Looks same as when used 'control' conditions




![Dendrogram VST transformed data, no batch correction (with controls indicated)](~/git/mtb_modules/Images/dendrogram_vst.png)


```{r batch_effect_correction}
#batch effect correction using limma; requirement to define batch effect 
        batch.table <- data.frame(study=conditions_df$study,condition=NA) 
        batch.table$condition <- conditions_df$condition 
        limma.expr<-removeBatchEffect(x=dds.vst_sum,
...
#Partial NA coefficients for 6483 probe(s) ?
        ...
        after.limma <- ggplot(df.vst.b,aes(x=PC1,y=PC2,color=dataset, shape=group)) + scale_shape_manual(values = 0:20) + geom_point(size=3) + xlab("PC1 (31.8%)") + ylab("PC2 (18.5%)")
        after.limma <- after.limma + scale_color_manual(values = toned_down_pal) + theme_bw()
        after.limma
        #PC2 PC3
        after.limma2 <- ggplot(df.vst.b,aes(x=PC2,y=PC3,color=dataset, shape=group)) + scale_shape_manual(values = 0:20) + geom_point(size=3) + xlab("PC2 (18.5%)") + ylab("PC3 (11.6%)")
        after.limma2 <- after.limma2 + scale_color_manual(values = toned_down_pal) + theme_bw()
        after.limma2
        #PC3 PC4
        after.limma3 <- ggplot(df.vst.b,aes(x=PC3,y=PC4,color=dataset, shape=group)) + scale_shape_manual(values = 0:20) + geom_point(size=3) + xlab("PC3 (11.6%)") + ylab("PC4 (8%)")
...
Maybe should try without pre-filtering for low expression? May contribute to batch effects if one dataset has more low expression signals than others? Try again without pre-filtering.
...
#batch effect correction using limma; requirement to define batch effect 
        ...
        limma.expr2<-removeBatchEffect(x=dds_vst_unfiltered,
                                    batch=batch.table$study,
                                    batch2=NULL,
                                    covariates=NULL,
                                    design=model.matrix(~ batch.table$condition)) 
        ...
        #generating PCA table post-limma
        datExpAdj2 <- t(limma.expr2)
        pca_limma2 <- prcomp(datExpAdj2)
        df.vst.un <- as.data.frame(pca_limma2$x)
        df.vst.un$group <- control.metadata$condition
        df.vst.un$dataset <- control.metadata$study
        summary(pca_limma2) 
#####
        #PCA plot for VST transformed data post-limma; PC1 PC2
        after.limma2 <- ggplot(df.vst.un,aes(x=PC1,y=PC2,color=dataset, shape=group)) + scale_shape_manual(values = 0:16) + geom_point(size=3) + xlab("PC1 (38%)") + ylab("PC2 (27%)")
        after.limma2 <- after.limma2 + scale_color_manual(values = toned_down_pal) + theme_bw()
        after.limma2
        #PC2 PC3
        after.limma21 <- ggplot(df.vst.b,aes(x=PC2,y=PC3,color=dataset, shape=group)) + scale_shape_manual(values = 0:16) + geom_point(size=3) + xlab("PC2 (27%)") + ylab("PC3 (22%)")
        after.limma21 <- after.limma21 + scale_color_manual(values = toned_down_pal) + theme_bw()
        after.limma21
        #PC3 PC4
        after.limma31 <- ggplot(df.vst.b,aes(x=PC3,y=PC4,color=dataset, shape=group)) + scale_shape_manual(values = 0:16) + geom_point(size=3) + xlab("PC3 (22%)") + ylab("PC4 (19%)")
        after.limma31 <- after.limma31 + scale_color_manual(values = toned_down_pal) + theme_bw()
        after.limma31
```

There is no apparent difference in pca plot or dendrograph either--filtering doesn't seem to affect batch effects.
...
#WGCNA 
        ...
        head(analysis)[,1:5]
        #defining trait data 
        traitData <- read.csv("mtb_conditions.txt")
        dim(traitData)
        names(traitData)
...
Just re-do it all with your own script and own variable names--this is too confusing. 

[Notes on VST](http://bioconductor.org/packages/release/bioc/manuals/DESeq2/man/DESeq2.pdf) 

better for larger datasets >30, this is larger, but not too large.
Not as good with datasets with large variations in sequencing depth.
...
**This is continued in pca_wcgna.Rmd**
